...
- **The Collection**: the app's home: browse all 951 amiibo, scan a local
...
The page renders the full database immediately: 951 amiibo across 32 series
(950 database entries plus the curated Happy Home Designer card set),
...
and every total (951 becomes 950), so an official-only collection can read
...
**The database** (`web/data/amiibo-db.js`, 950 entries across 32 series and
...
| Table | Holds | Rows |
|---|---|---|
| `AMIIBO_SERIES_SHORT` | series byte → short folder token | 32 |
| `AMIIBO_FILE_NAMES` | ID → filename unique within its series | 21 |
| `AMIIBO_SHORT_NAMES` | ID → abbreviated filename | 49 |

The last two carry only the rows that differ from `AMIIBO_NAMES`, so 21 and 49
rather than 950 apiece.
...
- `bundle.test.mjs`: the all-in-one format. Detection accepts a real bundle and
  ...
  limits across all 950 entries at several device roots; unpacking drops what you
...
- `collectionview.test.mjs` and `collectiongrid.test.mjs`: the collection, split
  into what it decides and what it draws. The grid is built for all 32 series
...
web/data/amiibo-db.js     950 amiibo IDs -> name/series/type/date (generated)
