...
- **The Collection**: the app's home: browse all 950 amiibo, scan a local
...
The page renders the full database immediately: 950 amiibo across 32 series
(949 database entries plus the curated Happy Home Designer card set),
...
and every total (950 becomes 949), so an official-only collection can read
...
**The database** (`web/data/amiibo-db.js`, 949 entries across 32 series and
...
- `bundle.test.mjs`: the all-in-one format. Detection accepts a real bundle and
  ...
  limits across all 949 entries at several device roots; unpacking drops what you
...
web/data/amiibo-db.js     949 amiibo IDs -> name/series/type/date (generated)
